...
- Reset DB and Run current migrations
  ```
  yarn db:reset
...
- Create an Entity & Module
  ```
  yarn medex g --entity <ENTITY NAME>
  yarn medex g --module <ENTITY NAME>
  ```
- Build - Transpile all files inside `dist` directory
...
- Add Migration to Module
  ...
  ```
  Within `src/modules/migrations` add the new migration to the MigrationModule
  ```

- Run Medex Migration
  ```
  yarn medex migrate --run
  ```
...
You must addd `ADMIN_CORS` in order to successfully authenticate the Admin portal. It's currently defaulted to `http://localhost:8080`.

To create a new admin user from the command line, run the following command:
